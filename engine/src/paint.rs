...
impl Paint {
  ...
  pub fn create_triangle2d(&mut self, points: [[f32; 2]; 3]) -> Painted {
    ...
    self.vertices.push(0f32);
    Painted { start: initial, length: 9 }
  }
  ...
  pub fn draw_triangle2d(&mut self, painted: &Option<Painted>, points: [[f32; 2]; 3]) -> Painted {
    if painted.is_some() {
      ...
      for i in 0..=2 {
        self.vertices.insert(i * 3 + unwrapped_painted.start + 0, points[i][0]);
        self.vertices.insert(i * 3 + unwrapped_painted.start + 1, points[i][1]);
        self.vertices.insert(i * 3 + unwrapped_painted.start + 2, 0f32);
      }
...
mod tests {
  ...
  fn create_triangle2d() {
    ...
    let result = paint.create_triangle2d([[-0.5f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.5f32]]);
  ...
  fn draw_new_triangle2d() {
    let mut paint = Paint::new(&0);
    let result = paint.draw_triangle2d(&None, [[-0.5f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.5f32]]);
    assert_eq!(paint.vertices[0], -0.5f32);
    assert_eq!(paint.vertices[1], -0.5f32);
    assert_eq!(paint.vertices[2], -0f32);
    assert_eq!(paint.vertices[3], 0.5f32);
  ...
  fn draw_existing_triangle2d() {
    let mut paint = Paint::new(&0);
    let painted = paint.draw_triangle2d(&None, [[-0.5f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.5f32]]);
    let result = paint.draw_triangle2d(&Some(painted), [[-0.1f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.1f32]]);
  ...
  fn draw_existing_second_triangle2d() {
    let mut paint = Paint::new(&0);
    paint.draw_triangle2d(&None, [[-0.5f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.5f32]]);
    let painted = paint.draw_triangle2d(&None, [[-0.5f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.5f32]]);
    let result = paint.draw_triangle2d(&Some(painted), [[-0.1f32, -0.5f32], [0.5f32, -0.5f32], [0.0f32, 0.1f32]]);
